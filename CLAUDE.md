...
| `storage` | `toggle_task` / `update_task_text` / `remove_task_line` / `append_task` / `append_task_to_quadrant` —— 通过 `tempfile::NamedTempFile` 原子写。`update_task_text` 逐字节保留行前的缩进 / bullet / checkbox 前缀；`remove_task_line` 删行前用 `parse_line` 二次校验目标行确实是任务行；拒绝空文本与多行输入 |
