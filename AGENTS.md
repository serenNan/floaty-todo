...
| `src/services/tauri-api.ts` | `api` 对象 —— 封装 `invoke` 命令 + 对话框选择器（`pickFolder` / `pickMarkdownFile`）+ 事件监听器（`tasks-updated` / `sources-changed` / `request-manage-sources` / `history-updated` / `history-seen-changed`）+ `emitHistorySeen()` 前端广播事件（Tauri 2 `@tauri-apps/api/event.emit` 默认跨窗口，不需要 Rust 转发） |
...
| `src/stores/history.ts` | `useHistoryStore` —— `events` / `cursorId` / `loading` / `error` / `hasRedo` / `lastSeenAt` / `unseenExternal`；`refresh` / `undo` / `redo` / `jumpTo(id, confirmExternal)` / `markSeen()` / `syncLastSeenFromStorage()`。`jumpTo` 失败时 re-throw 原 error 让 caller 看 `.code`，但 `EXTERNAL_IN_UNDO_RANGE` 不出 toast（业务确认路径，caller 会 re-invoke）。`unseenExternal` = 截止 `lastSeenAt`（localStorage `floaty.history.lastSeenAt`）之后的 `external_edit` 事件数，主窗口 footer 历史按钮据此挂红点。`markSeen()` 同时调 `api.emitHistorySeen()` 广播 `history-seen-changed` 到主窗口，主窗口 listener 调 `syncLastSeenFromStorage()` 重读 localStorage、`unseenExternal` 重新计算 → 0 → 红点消失。所有 undo / redo / jumpTo 成功后会 emit toast；undo/redo 返回 null（无可撤销/重做）时 emit info toast，不报错 |
| `src/views/HistoryView.vue` | 独立 history 窗口的根组件（Tauri label="history"）。左侧 timeline 事件流（source 色块、外部编辑半透明、cursor 之后的事件灰显），右侧 EVENT detail（before/after pre 渲染 diff，`跳到此处` 按钮）；jump 区间含外部编辑时通过 `useConfirm` 弹危险确认（不走 `window.confirm`）；含 `<ConfirmDialog />` + `<ToastContainer />` mount（history 窗口的 confirm / toast 都得就近渲染，触发动作的窗口出自己的反馈）。`onMounted` 里 `await history.refresh()` 之后调 `history.markSeen()` —— 打开历史窗口即视为「已看过所有外部编辑」，跨窗口广播让主窗口红点消失。**onMounted 里没有 show()** —— 窗口由 lib.rs 在 app 启动时预创建为隐藏，Rust 的 `open_history_window` 负责 show + focus，所以这里启动时不应让自己冒出来 |
...
| `src/composables/useToast.ts` | 模块级单例 toast 队列 + API：`toast.success(msg)` / `toast.warning(msg)` / `toast.error(msg)` / `toast.info(msg)`，可选 `{ duration }` 覆盖默认（success 2s / info 3s / warning 4s / error 6s）。`MAX_VISIBLE = 3` —— 超出排队即丢最旧。`useToastState()` 给组件用：返回 `items` ref + `dismiss(id)` / `pause(id)` / `resume(id)`（hover 暂停剩余计时）。i18n 接入注意：Pinia store 里**不要**用 `useI18n()`（store 在 `main.ts` 模块级 `useXxxStore(pinia)` 触发 setup，没有 active component context），改用 `import { i18n } from '../i18n'; const t = i18n.global.t;` |
| `src/components/ConfirmDialog.vue` | 由 `useConfirm` 驱动的 Teleport 模态；点遮罩 / Esc 取消，confirm 按钮 focus-trap，危险动作有红色变体 |
| `src/components/ToastContainer.vue` | 渲染 `useToastState().items` 的全局组件，必须在 `App.vue` + `HistoryView.vue` 两个 root 各挂一次（per-window 队列）。`<Teleport to="body">` + `<TransitionGroup name="toast">`：进场 translateY 弹性曲线，离场 scale + fade（不 translate，避免和兄弟上滑撞车），`.toast-move` 给兄弟做 FLIP 平滑滑动。容器 `position: fixed; bottom: 52px; left: 50%`（压在 footer 上方居中），`align-items: center` 让不同宽度的 toast 各自居中堆叠。单条 toast `inline-flex` + `justify-content: center` —— 宽度跟内容走（短消息 → 窄气泡，长消息 → 撑到 max-width: 100%），无 `min-width`；× 按钮 `position: absolute` 贴右边只在 hover 显示。变体填整块色：success 绿 / warning 琥珀+深字 / error 红 / info 石板灰 |
...
| `src/components/TaskList.vue` | 分组任务视图（按 config 顺序逐个渲染 `SourceGroup`）。顶部 `.toolbar`：搜索框（`provide('searchQuery', ref)` 给后代 FileGroup / TaskItem 用，激活时强制展开所有匹配组、并在 TaskItem 内部分词高亮）+ 「+ 添加源」hover 菜单（Folder / File）+ 全部折叠/展开。footer：左下 ⚙ Settings + 计数 + 🕒 历史按钮 + 可选 hub 快捷动作（reveal / vscode / claude_code）+ 📌 钉住 + ↻ 刷新。🕒 历史按钮 `::after` 右上绿点 = `history.hasRedo`、`::before` 左上红点 = `history.unseenExternal > 0`，两个点位置对称可同时存在；`title` 用 computed 拼上「N 条未看过的外部修改」。任务添加入口已移到每个 SourceGroup header 的「+」按钮 |
