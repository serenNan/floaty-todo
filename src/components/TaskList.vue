...
const groupedTasks = computed(() => {
  ...
  for (const tk of tasks.sortedTasks) {
    if (q && !tk.text.toLowerCase().includes(q)) continue;
    buckets.get(tk.source_id)?.push(tk);
  }
  const groups = settings.sources.map(s => ({ source: s, tasks: buckets.get(s.id) ?? [] }));
  return q ? groups.filter(g => g.tasks.length > 0) : groups;
});
...
/* Compact top toolbar — task input moved into per-source QuickAdd dialogs,
   so this row only carries source-management actions (add source, collapse). */
.toolbar {
  display: flex;
  align-items: center;
